...
- [`snap/SPEC.md`](snap/SPEC.md) — the canonical behavioral contract.
- [`snap/tests/`](snap/tests/) — language-neutral YAML acceptance tests.
- [`snap/TEST-HARNESS.md`](snap/TEST-HARNESS.md) and [`snap/test-harness/`](snap/test-harness/) —
  the extensible process/filesystem/HTTP test format and driver.
- [`snap/local_test/`](snap/local_test/) — local testing walkthrough guide and demo scripts.
- `snap/<language>/` — the selected scaffold's location in an attendee archive.
- [`snap/run`](snap/run) — the bundled launcher; it selects the most recently modified
  available language implementation, or accepts `--lang`.
- [`snap/verify`](snap/verify) — the public acceptance-test entry point.
...
./snap/run config --global contributor.id you@example.com
cd /tmp/example
...
```bash
./snap/verify --candidate /path/to/snap
```
