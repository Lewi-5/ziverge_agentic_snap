...
- Repository imports are source-neutral and collision-safe: merge and cross-repository diff load `ValidatedRepository` values through one source port, compare every common typed patch dot, and merge/revert apply a complete pure tree-mutation plan before atomically publishing prepared metadata.
