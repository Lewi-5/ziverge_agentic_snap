...
| M5 | Full Validation, Deterministic Replay, and OT | M1, M3, M4 | `In Progress` | Codex (TS) | 15, 23, 27 | 2026-09-04: `npm run check` passes; 53/53 M5-focused tests pass (including 24 seeded graphs × 8 permutations); public 15, 23, and 27 pass; loader regressions 04, 05, 06, and 25 pass. Full emitted suite: 312/317 pass, 4 host-permission skips, 1 unrelated M7 grammar-test failure. | Commit `969ffb6` implements the single `ValidatedRepository` boundary, exact closure/semantic validation, arbitrary known-version selection, dynamic ready scheduling, exact-base replay, split-cursor aggregate OT, namespace/path conflict rules, warning ordering/deduplication, and status/log/diff/commit integration; the old linear validator is now only a compatibility export. Remaining regression evidence is environmental/harness-side: 03 stops before candidate execution because the harness treats nested JSON's closing `}}` as interpolation, and 08 stops at Windows symlink creation with EPERM. Keep M5 in progress under the strict completion rule until those can run on a compatible host; merge-facing scenarios remain M6. |
