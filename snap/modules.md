...
| M5 | Full Validation, Deterministic Replay, and OT | M1, M3, M4 | `In Progress` | Codex (TS) | 15, 23, 27 | 2026-09-04: `npm run check` passes (production/test TypeScript plus zero-warning type-aware lint); 53/53 M5-focused emitted-JavaScript tests pass, including 24 seeded causal graphs across 8 storage permutations each and two real-adapter validation-boundary tests. | Commit `969ffb6` implements the single `ValidatedRepository` boundary, exact closure/semantic validation, arbitrary known-version selection, dynamic ready scheduling, exact-base replay, split-cursor aggregate OT, namespace/path conflict rules, warning ordering/deduplication, and status/log/diff/commit integration; the old linear validator is now only a compatibility export. Public 15/23/27 remain outstanding because the packaged Bash verifier cannot run on this Windows host (`spawn EFTYPE`/path-normalization failure); rerun those three filters on Linux/macOS/WSL before marking complete. Merge-facing scenarios remain M6. |
