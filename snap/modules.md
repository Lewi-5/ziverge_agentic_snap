...
| M7 | CLI Hardening and Presentation | M1–M6 | `In Progress` | Unassigned (last touched by Claude, TS) | 14, 24 plus internal presentation gate | 2026-09-04: `npx tsc --noEmit -p tsconfig.test.json` and `npx eslint "src/**/*.ts" --max-warnings 0` clean; 325/329 unit tests pass via `tsc -p tsconfig.test.json --noEmit false --outDir <dir> && node --test` (4 pre-existing symlink-privilege skips); `./snap/verify --lang ts --filter 24-cli-grammar-matrix` passes in full; `./snap/verify --lang ts --filter 14-cli-errors` fails only at the M6-owned `revert` step. | All CLI grammar/presentation primitives (`ansi.ts`, `command-request.ts`, `grammar.ts`, `presentation.ts`, `render-terminal.ts`) and their wiring into `main.ts`/`dispatch.ts` (presentation resolution + plain/terminal renderer selection for every command) are implemented and exercised end-to-end. This session additionally rewrote `cli/commands/diff.ts` to dispatch on `parseCliArgs` instead of an ad hoc check, fixing the real scenario 14/24 diff-usage-diagnostic bug, and fixed two stale M1-era test invariants plus one stale diff grammar test that presentation resolution's per-command `SNAP_COLOR`/`NO_COLOR` read had broken. Full details in `module_plans/module7PROGRESS.md`. Remaining: wire `merge`/`revert` into `dispatch.ts`'s `COMMANDS` map once M6 lands (M7 cannot reach `Complete` before M6 per its exit gate); scenario 28 stays M8 scope. Note: this working tree is shared with at least one other concurrently-running agent session — re-check `git log`/`git diff` against current `HEAD` before assuming what is or isn't done. |
