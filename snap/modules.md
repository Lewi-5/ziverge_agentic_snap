...
| M5 | Full Validation, Deterministic Replay, and OT | M1, M3, M4 | `In Progress` | Codex (TS) | 15, 23, 27 | 2026-09-04: `npm run check` passes; 53/53 M5-focused emitted-JavaScript tests pass, including 24 seeded causal graphs across 8 storage permutations each; public scenarios 15, 23, and 27 all pass end-to-end. | Commit `969ffb6` implements the single `ValidatedRepository` boundary, exact closure/semantic validation, arbitrary known-version selection, dynamic ready scheduling, exact-base replay, split-cursor aggregate OT, namespace/path conflict rules, warning ordering/deduplication, and status/log/diff/commit integration; the old linear validator is now only a compatibility export. M3 loader regressions 04 and 25 pass; 05/06 are currently red only because separate uncommitted M7 work changed zero-count hunk starts against the public goldens, 03 is blocked by the harness treating nested JSON's closing `}}` as interpolation, and 08 is host-blocked by Windows symlink EPERM. Per the completion rule, keep M5 in progress until those regression gates are green; merge-facing scenarios remain M6. |
