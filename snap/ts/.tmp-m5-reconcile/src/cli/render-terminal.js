...
import { ANSI_BOLD, ANSI_CYAN, ANSI_DIM, ANSI_GREEN, ANSI_MAGENTA, ANSI_RED, ANSI_YELLOW, styleAnsi, } from "./ansi.js";
function escapeLogMessage(message) {
    return message.replaceAll("\\", "\\\\").replaceAll("\t", "\\t").replaceAll("\n", "\\n");
}
function renderTokenLineTerminal(line) {
...
export function renderDiffTerminal(records) {
    let output = "";
...
function renderStatusTerminal(versionStr, rows) {
...
function renderLogTerminal(entries) {
 ...
 * S(31, "✗ " + <error>) + "\n"
 */
export function formatCliErrorLineTerminal(plainErrorLine) {
 ...
 * S(33, "⚠") + " " + S(33, "<detail>") + "\n"
 */
export function formatCliWarningLineTerminal(detail) {
 ...
 */
export function renderCommandResultTerminal(result) {
    switch (result.kind) {
        case "initialized": {
