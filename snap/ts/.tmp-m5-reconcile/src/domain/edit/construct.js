...
export function constructEdit(value, base) {
    if (!Array.isArray(value))
        return err(domainError("validation", "edit must be an array"));
    if (value.length === 0) {
    ...
    for (let index = 0; index < value.length; index += 1) {
        ...
        if (kind === previousKind)
            return err(domainError("validation", `adjacent ${kind} operations are forbidden`));
        previousKind = kind;
