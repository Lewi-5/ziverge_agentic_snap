...
import { consumeOperation, INITIAL_OPERATION_CURSOR, operationHead } from "./operation-stream.js";
...
export function transformEdit(incoming, context) {
    const output = [];
    let incomingCursor = INITIAL_OPERATION_CURSOR;
    let contextCursor = INITIAL_OPERATION_CURSOR;
    for (;;) {
        const contextHead = operationHead(context, contextCursor);
        if (contextHead !== undefined && contextHead.remaining <= 0)
        ...
        if (incomingHead !== undefined && incomingHead.remaining <= 0)
            return invalidStream();
        ...
        if (incomingHead === undefined && contextHead === undefined)
            break;
        if (incomingHead === undefined || contextHead === undefined)
            return invalidStream();
