...
import { createTrackedPath } from "../tree/path.js";
...
function requireExactKeys(value, keys, context) {
    ...
    if (unknown !== undefined)
        return err(domainError("validation", `${context} has unknown field: ${unknown}`));
    const missing = keys.find((key) => !Object.hasOwn(value, key));
    if (missing !== undefined)
        return err(domainError("validation", `${context} is missing field: ${missing}`));
    return ok(undefined);
}
function decodeVersion(value, field) {
    ...
    for (const rawEntry of value) {
        ...
        if (!Number.isSafeInteger(revisionRaw) || revisionRaw < 1 || revisionRaw > MAX_REVISION) {
        ...
        previousId = idRaw;
        components.push({ contributorId: contributorId.value, revision: revisionRaw });
    }
...
function decodeChange(value) {
    ...
    if (type === "text") {
        ...
        if (typeof pathRaw !== "string")
            return err(domainError("validation", "change path must be a string"));
        const path = createTrackedPath(pathRaw);
        if (!path.ok)
            return err(domainError("validation", `path is invalid: ${path.error.detail}`));
        if (!("edit" in value))
            return err(domainError("validation", "text change is missing 'edit'"));
...
function decodeChanges(value) {
    if (!Array.isArray(value))
        return err(domainError("validation", "patch changes must be an array"));
    if (value.length === 0)
        return err(domainError("validation", "patch changes is empty"));
    const changes = [];
    let previousPath;
    for (const rawChange of value) {
        ...
        if (previousPath !== undefined) {
            ...
            if (order === 0)
                return err(domainError("validation", `patch changes contains more than one change for path '${decoded.value.path}'`));
            if (order > 0)
                return err(domainError("validation", "patch changes must be sorted by path"));
        }
...
function decodePatch(value) {
    ...
    if (typeof authorRaw !== "string")
        return err(domainError("validation", "patch author must be a string"));
    ...
    if (typeof messageRaw !== "string")
        return err(domainError("validation", "patch message must be a string"));
 ...
 */
export function decodeRepositoryDocument(value) {
    if (!isPlainObject(value)) {
