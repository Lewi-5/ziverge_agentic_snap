 ...
 */
export function createVersion(components) {
    ...
    for (const component of components) {
        if (!isValidContributorId(component.contributorId)) {
            return err(domainError("validation", `invalid contributor id '${escapeControlCharacters(component.contributorId)}'`));
        }
        if (!Number.isInteger(component.revision) || component.revision < 1 || component.revision > MAX_REVISION) {
            return err(domainError("validation", `invalid revision for contributor '${component.contributorId}'`));
        }
    ...
    const sorted = sortByUnsignedUtf8(components, (component) => component.contributorId).map((component) => Object.freeze({ ...component }));
