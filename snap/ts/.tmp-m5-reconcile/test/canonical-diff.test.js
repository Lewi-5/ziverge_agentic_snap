...
function sequences(alphabet, maximumLength) {
    const output = [[]];
    for (let length = 1; length <= maximumLength; length += 1) {
        const prior = output.filter((item) => item.length === length - 1);
        for (const prefix of prior)
            for (const token of alphabet)
                output.push([...prefix, token]);
    }
    return output;
}
function referenceDistance(a, b, i = 0, j = 0) {
    if (i === a.length)
        return b.length - j;
    if (j === b.length)
        return a.length - i;
    if (a[i] === b[j])
        return referenceDistance(a, b, i + 1, j + 1);
...
test("exhaustive short corpus is minimal, complete, coalesced, and round trips", () => {
    ...
    for (const oldTokens of corpus)
        for (const newTokens of corpus) {
            ...
            if (applied.ok)
                assert.deepEqual(applied.value, newTokens);
        }
});
