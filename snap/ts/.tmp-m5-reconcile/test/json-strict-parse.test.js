...
function expectError(text, detailIncludes) {
    const result = parseJsonStrict(text);
    assert.equal(result.ok, false, `expected '${text}' to be rejected`);
    if (!result.ok) {
        assert.ok(result.error.detail.includes(detailIncludes), `expected detail '${result.error.detail}' to include '${detailIncludes}'`);
    }
}
