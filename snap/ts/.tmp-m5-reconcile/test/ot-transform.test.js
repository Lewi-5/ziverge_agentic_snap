...
function transformed(incoming, context) {
...
test("OT splits unequal retain and delete counts in both directions", () => {
    assert.deepEqual(transformed([{ retain: 5 }, { delete: 2 }], [{ retain: 2 }, { delete: 2 }, { retain: 3 }]), [{ retain: 3 }, { delete: 2 }]);
    assert.deepEqual(transformed([{ delete: 2 }, { retain: 5 }], [{ retain: 5 }, { delete: 2 }]), [{ delete: 2 }, { retain: 3 }]);
});
test("context insert wins priority over incoming insert at one cursor", () => {
    const script = transformed([{ insert: ["incoming\n"] }, { retain: 1 }], [{ insert: ["context\n"] }, { retain: 1 }]);
    assert.deepEqual(script, [{ retain: 1 }, { insert: ["incoming\n"] }, { retain: 1 }]);
    const applied = applyEdit(["context\n", "base\n"], script);
    assert.equal(applied.ok, true);
    if (applied.ok)
        assert.deepEqual(applied.value, ["context\n", "incoming\n", "base\n"]);
});
test("context insert survives an incoming delete and trailing inserts are preserved", () => {
    assert.deepEqual(transformed([{ delete: 1 }, { insert: ["p-tail"] }], [{ insert: ["q\n"] }, { retain: 1 }, { insert: ["q-tail"] }]), [{ retain: 1 }, { delete: 1 }, { retain: 1 }, { insert: ["p-tail"] }]);
});
test("OT coalesces adjacent output created by stream splitting", () => {
    assert.deepEqual(transformed([{ retain: 3 }], [{ retain: 1 }, { insert: ["x\n"] }, { retain: 2 }]), [{ retain: 4 }]);
});
...
test("OT does not mutate either input script", () => {
    const incoming = Object.freeze([{ retain: 1 }, { insert: Object.freeze(["p\n"]) }]);
    const context = Object.freeze([{ retain: 1 }, { insert: Object.freeze(["q\n"]) }]);
