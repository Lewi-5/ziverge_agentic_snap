...
import { renderCommandResultTerminal, formatCliErrorLineTerminal, formatCliWarningLineTerminal, } from "../src/cli/render-terminal.js";
...
test("renders committed, reverted, and merged in terminal mode", () => {
    const ver = getVersion(createVersion([
        { contributorId: "alice@example.com", revision: 1 },
    ]));
    const committed = renderCommandResultTerminal({ kind: "committed", version: ver });
    assert.equal(committed, `${styleAnsi(ANSI_GREEN, "✓")} ${styleAnsi(ANSI_BOLD, "Committed")} ${styleAnsi(ANSI_CYAN, "(alice@example.com->1)")}\n`);
    const reverted = renderCommandResultTerminal({ kind: "reverted", version: ver });
    assert.equal(reverted, `${styleAnsi(ANSI_GREEN, "✓")} ${styleAnsi(ANSI_BOLD, "Reverted")} ${styleAnsi(ANSI_CYAN, "(alice@example.com->1)")}\n`);
    const merged = renderCommandResultTerminal({ kind: "merged", version: ver });
    assert.equal(merged, `${styleAnsi(ANSI_GREEN, "✓")} ${styleAnsi(ANSI_BOLD, "Merged")} ${styleAnsi(ANSI_CYAN, "(alice@example.com->1)")}\n`);
});
...
test("renders log in terminal mode with double LF spacing between entries", () => {
    const ver1 = getVersion(createVersion([{ contributorId: "a@x", revision: 1 }]));
    const ver2 = getVersion(createVersion([{ contributorId: "a@x", revision: 2 }]));
    const result = renderCommandResultTerminal({
        kind: "log",
        entries: [
    ...
    const entry1 = `${styleAnsi(ANSI_CYAN, "●")} ${styleAnsi(ANSI_BOLD, "second\\nline")}\n` +
        `  ${styleAnsi(ANSI_CYAN, "(a@x->2)")} ${styleAnsi(ANSI_DIM, "by")} ${styleAnsi(ANSI_MAGENTA, "a@x")}\n`;
    const entry2 = `${styleAnsi(ANSI_CYAN, "●")} ${styleAnsi(ANSI_BOLD, "first")}\n` +
        ...
        records: [
            {
                ...
                lines: [
                    ...
                    { kind: "insert", token: "new" }, // missing final newline
                ],
            },
            {
                kind: "binary",
