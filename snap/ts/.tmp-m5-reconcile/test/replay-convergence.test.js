...
function editFor(seed, author) {
    ...
    if ((seed & 1) === 0) {
        ...
        if (position < 4)
            operations.push({ retain: 4 - position });
        return operations;
    }
    const deleted = seed % 4;
    const operations = [];
    if (deleted > 0)
        operations.push({ retain: deleted });
    operations.push({ delete: 1 });
    if (deleted < 3)
        operations.push({ retain: 3 - deleted });
    return operations;
}
...
function assumeValidated(document) {
    return { document };
}
function treeSnapshot(repository) {
    const result = materializeVersion(repository, repository.document.frontier);
    if (!result.ok)
        throw new Error(result.error.detail);
    return [...result.value.tree].map(([path, bytes]) => [path, [...bytes]]);
}
function validatedPatches(seed) {
    ...
    for (let seed = 1; seed <= 24; seed += 1) {
        ...
        for (let permutation = 0; permutation < 8; permutation += 1) {
            ...
            if (order.ok && expectedOrder.ok) {
                assert.deepEqual(order.value.map((patch) => `${patch.author}:${String(patch.revision)}`), expectedOrder.value.map((patch) => `${patch.author}:${String(patch.revision)}`));
            }
        }
    }
});
test("every patch result materializes its complete causal closure", () => {
    ...
    for (const patch of repository.document.patches) {
        ...
        if (result.ok && patch.author !== "a@x")
            assert.equal(result.value.tree.has("f"), true);
    }
});
