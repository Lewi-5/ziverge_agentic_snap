...
const EMPTY_DOCUMENT = Object.freeze({ format: 1, frontier: EMPTY_VERSION, patches: Object.freeze([]) });
...
test("a failure between the temp write and the rename leaves the original file byte-unchanged and cleans up the temp file", async () => {
    ...
    try {
        ...
        await assert.rejects(publishRepository(root, EMPTY_DOCUMENT, { fileSystem }), /simulated failure between write and rename/);
