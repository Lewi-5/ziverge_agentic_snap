...
test("full validator rejects gaps, unreachable extras, missing dependencies, and cycles", () => {
    ...
    if (!gap.ok)
        assert.match(gap.error.detail, /missing a@x/);
    ...
    if (!unreachable.ok)
        assert.match(unreachable.error.detail, /^unreachable patch:/);
    ...
    if (!missing.ok)
        assert.match(missing.error.detail, /missing dependency/);
    ...
    if (!cycle.ok)
        assert.equal(cycle.error.detail, "cyclic or incomplete patch history");
});
test("exact-base semantics reject invalid creates, edits, deletes, no-ops, and prefix results", () => {
    ...
    if (!deleteAbsent.ok)
        assert.equal(deleteAbsent.error.detail, "delete of absent path: f");
    const noOp = validate({
        format: 1,
        frontier: [["a@x", 2]],
        patches: [
    ...
    if (!noOp.ok)
        assert.match(noOp.error.detail, /no-op change/);
    ...
    if (!prefix.ok)
        assert.match(prefix.error.detail, /tree paths conflict/);
...
test("schema diagnostics expose strict layer failures", () => {
    ...
    if (!unknown.ok)
        assert.equal(unknown.error.detail, "repository has unknown field: unknown");
    ...
    if (!emptyChanges.ok)
        assert.match(emptyChanges.error.detail, /changes is empty/);
});
