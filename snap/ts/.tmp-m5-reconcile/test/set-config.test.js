...
function discoveryReturning(root) {
    return { findRepositoryRoot: () => Promise.resolve(root) };
}
function environmentWith(home) {
...
test("local write: valid id and repository present writes .snap/config.json with 2-space indent and LF", async () => {
    ...
    const result = await setConfig({ cwd: CWD, global: false, contributorId: "alice@example.com" }, { fileSystem, repositoryDiscovery: discoveryReturning(repoRoot), environment: environmentWith(undefined) });
...
test("local write overwrites existing content without reading or parsing it first", async () => {
    ...
    const result = await setConfig({ cwd: CWD, global: false, contributorId: "alice@example.com" }, {
...
test("local write outside a repository fails and makes zero write calls", async () => {
    const fileSystem = recordingFileSystem();
    const result = await setConfig({ cwd: CWD, global: false, contributorId: "alice@example.com" }, { fileSystem, repositoryDiscovery: discoveryReturning(null), environment: environmentWith(undefined) });
...
test("global write: valid id and HOME set writes $HOME/.snapconfig.json without repository discovery", async () => {
    ...
    const trackingDiscovery = {
        findRepositoryRoot: () => {
            discoveryCalls += 1;
            return Promise.resolve(null);
        },
    };
    const result = await setConfig({ cwd: CWD, global: true, contributorId: "alice@example.com" }, { fileSystem, repositoryDiscovery: trackingDiscovery, environment: environmentWith(home) });
    assert.equal(result.ok, true);
...
test("global write with HOME unset fails with 'global configuration is unavailable' and writes nothing", async () => {
    const fileSystem = recordingFileSystem();
    const result = await setConfig({ cwd: CWD, global: true, contributorId: "alice@example.com" }, { fileSystem, repositoryDiscovery: discoveryReturning(null), environment: environmentWith(undefined) });
    assert.equal(result.ok, false);
    if (!result.ok) {
...
test("global write with HOME empty fails with 'global configuration is unavailable'", async () => {
    const fileSystem = recordingFileSystem();
    const result = await setConfig({ cwd: CWD, global: true, contributorId: "alice@example.com" }, { fileSystem, repositoryDiscovery: discoveryReturning(null), environment: environmentWith("") });
    assert.equal(result.ok, false);
    assert.equal(fileSystem.writes.size, 0);
});
test("invalid contributor id fails before environment lookup or repository discovery, writing nothing", async () => {
    ...
    const result = await setConfig({ cwd: CWD, global: true, contributorId: "two@@x" }, { fileSystem, repositoryDiscovery: trackingDiscovery, environment: trackingEnvironment });
    assert.equal(result.ok, false);
    if (!result.ok) {
