...
function renderTokenLine(line: DiffTokenLine): string {
  const prefix = line.kind === "context" ? " " : line.kind === "delete" ? "-" : "+";
  const rendered = `${prefix}${line.token}`;
  return line.token.endsWith("\n") ? rendered : `${rendered}\n\\ No newline at end of file\n`;
}

export function renderDiffPlain(records: readonly DiffRecord[]): string {
  let output = "";
  for (const record of records) {
    ...
    for (const line of record.lines) output += renderTokenLine(line);
  }
  return output;
}
