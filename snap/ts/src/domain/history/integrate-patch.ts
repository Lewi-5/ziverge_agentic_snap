...
export function integratePatch(base: FileTree, current: FileTree, patch: Patch): Result<PatchIntegration, DomainError> {
  const authoredTarget = applyAuthoredPatch(base, patch);
  if (!authoredTarget.ok) return authoredTarget;

  const presentTargets = new Map<string, Uint8Array>();
  const authoredDeletions = new Set<string>();
  for (const change of patch.changes) {
    const content = authoredTarget.value.get(change.path);
    if (content === undefined) authoredDeletions.add(change.path);
    else presentTargets.set(change.path, content);
  }
  ...
  const warnings: WarningFact[] = [...namespace.warnings];

  for (const change of patch.changes) {
    if (namespace.settledIncomingPaths.has(change.path)) continue;
    ...
    if (pathContentsEqual(baseContent, currentContent)) {
      ...
      if (baseClassified.kind === "text" && currentClassified.kind === "text" && authoredClassified.kind === "text") {
        ...
        if (conflict.warning !== undefined) warnings.push(conflict.warning);
      }
    } else {
      const conflict = resolvePathConflict(change.path, baseContent, currentContent, authoredContent, change);
      resolved = conflict.content;
    ...
    if (resolved === undefined) removals.add(change.path);
    else installations.set(change.path, resolved);
  }

  const next = new Map(current);
  for (const path of removals) next.delete(path);
  for (const [path, content] of installations) next.set(path, content);
