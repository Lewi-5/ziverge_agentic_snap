...
test("renders canonical whole-file text diff and both missing-final-newline markers", () => {
  ...
  assert.equal(renderDiffPlain(records),
    "--- a/note.txt\n" +
    "+++ b/note.txt\n" +
    "@@ -1,2 +1,2 @@\n" +
    " same\n" +
...
test("renders create/delete headers, empty text hunk, and a binary notice", () => {
  const records = buildDiffRecords(
    new Map<string, Uint8Array>([["gone", bytes("bye\n")], ["raw", new Uint8Array([0])]]),
    new Map<string, Uint8Array>([["empty", bytes("")]]),
  );
  assert.equal(renderDiffPlain(records),
    "--- /dev/null\n+++ b/empty\n@@ -1,0 +1,0 @@\n" +
    "--- a/gone\n+++ /dev/null\n@@ -1,1 +1,0 @@\n-bye\n" +
...
test("semantic records retain line kinds for later presentation", () => {
  ...
  if (records[0]?.kind === "text") assert.deepEqual(records[0].lines.map((line) => line.kind), ["delete", "insert"]);
});

test("matches the public repeated-line and unterminated-create golden", () => {
  const records = buildDiffRecords(
    new Map([["repeated.txt", bytes("a\nb\na\n")]]),
...
test("matches the public CRLF, NUL-binary, and Unicode portability golden", () => {
  const records = buildDiffRecords(new Map(), new Map<string, Uint8Array>([
