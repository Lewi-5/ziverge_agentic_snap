...
import { createContributorId, type ContributorId } from "../src/domain/version/contributor-id.js";
import { createVersion } from "../src/domain/version/construct.js";
import type { Version } from "../src/domain/version/types.js";

function id(value: string): ContributorId {
  const result = createContributorId(value);
  if (!result.ok) throw new Error(result.error.detail);
  return result.value;
}

function version(...components: readonly (readonly [ContributorId, number])[]): Version {
...
test("scheduler handles empty and linear histories", () => {
  ...
  if (scheduled.ok) assert.deepEqual(scheduled.value.map((patch) => patch.label), ["a1", "a2"]);
});
...
test("scheduler recomputes readiness and orders concurrent results by Snap order", () => {
  ...
  assert.equal(scheduled.ok, true);
  if (scheduled.ok) assert.deepEqual(scheduled.value.map((patch) => patch.label), ["b1", "a1", "c1", "a2"]);
});
...
test("scheduler output is independent of storage permutation", () => {
  ...
  for (const input of [[a1, b1, a2], [a2, a1, b1], [b1, a2, a1]]) {
    ...
    assert.equal(scheduled.ok, true);
    if (scheduled.ok) assert.deepEqual(scheduled.value.map((patch) => patch.label), ["b1", "a1", "a2"]);
  }
});
